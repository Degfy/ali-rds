...
describe('client.test.js', function() {
  ...
  describe('query(), queryOne()', function() {
    ...
    it('should select 2 rows', function*() {
      const rows = yield this.db.query('select * from ?? where email=? order by id', [table, prefix + 'm@fengmk2.com']);
    ...
    it('should select 1 row', function*() {
      const row = yield this.db.queryOne('select * from ?? where email=? order by id', [table, prefix + 'm@fengmk2.com']);
  ...
  describe('transactions', function() {
    ...
    it('should insert 2 rows in a transaction', function*() {
      ...
      const rows = yield this.db.query('select * from ?? where email=? order by id', [table, prefix + 'm@transaction.com']);
      assert.equal(rows.length, 2);
    ...
    it('should use db.beginTransaction()', function*() {
      ...
      const rows = yield this.db.query('select * from ?? where email=? order by id', [table, prefix + 'm@beginTransaction.com']);
      assert.equal(rows.length, 2);
    ...
    it('should rollback when query fail', function*() {
      ...
      const rows = yield this.db.query('select * from ?? where email=? order by id', [table, prefix + 'm@transaction-fail.com']);
  ...
  describe('beginTransactionScope(scope)', function() {
    ...
    it('should insert 2 rows in a transaction', function*() {
      try {
        const result = yield this.db.beginTransactionScope(async conn => {
          return co(function*() {
            yield conn.query('insert into ??(name, email, gmt_create, gmt_modified) \
        ...
        const rows = yield this.db.query('select * from ?? where email=? order by id', [table, prefix + 'm@beginTransactionScope1.com']);
        assert.equal(rows.length, 2);
    ...
    it('should rollback when query fail', function*() {
      ...
      const rows = yield this.db.query('select * from ?? where email=? order by id', [table, prefix + 'm@beginTransactionScope-fail.com']);
      assert.equal(rows.length, 0);
    });
    ...
    describe('beginTransactionScope(fn, ctx)', function() {
      it('should insert 7 rows in a transaction with ctx', function*() {
        ...
        function* hiInsert() {
          return yield db.beginTransactionScope(async conn => {
            return co(function*() {
              yield conn.query('insert into ??(name, email, gmt_create, gmt_modified) \
        ...
        const result = yield db.beginTransactionScope(
          async conn => {
            return co(function*() {
              yield conn.query('insert into ??(name, email, gmt_create, gmt_modified) \
        ...
        const rows = yield db.query('select * from ?? where email=? order by id', [table, prefix + 'm@beginTransactionScopeCtx1.com']);
      ...
      it('should auto rollback on fail', function*() {
        ...
        try {
          yield db.beginTransactionScope(async conn => {
            return co(function*() {
              yield conn.query('insert into ??(name, email, gmt_create, gmt_modified) \
              values(?, ?, now(), now())', [table, prefix + 'beginTransactionScope-ctx-fail1', prefix + 'm@beginTransactionScope-ctx-fail1.com']);
              yield conn.query('insert into ??(name, email, gmt_create, gmt_modified) \
        ...
        const rows = yield db.query('select * from ?? where email=? order by id', [table, prefix + 'm@beginTransactionScope-ctx-fail1.com']);
        assert.equal(rows.length, 0);
        assert.equal(ctx._transactionConnection, null);
